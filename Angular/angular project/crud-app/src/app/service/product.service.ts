...
export class ProductService {
  ...
  getProductById(id: number): Observable<Product> {
    return this.http.get<Product>(`http://localhost:8093/product/api.2.0/retrieve/${id}`);
  }
  ...
  deleteProductById(id: number): Observable<Product> {
    return this.http.get<Product>(`http://localhost:8093/product/api.2.0/delete/${id}`);
  }
